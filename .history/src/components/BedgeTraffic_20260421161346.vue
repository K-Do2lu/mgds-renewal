...
<template>
    <div class="bedge traffic" v-for="item in trafficBedge" :key="item.title">
        <span class="traffic-title" :style="{ background: item.bg, border: `1px solid ${item.border}`, color: item.text }"> {{ item.title }} </span>
        <span class="traffic-number" :style="{ color: item.text }"> {{ item.number }} </span>
    </div>
</template>
